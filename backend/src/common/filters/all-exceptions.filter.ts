...
export class AllExceptionsFilter implements ExceptionFilter {
    ...
    constructor(private readonly configService: ConfigService) {}
        ...
        if (statusCode >= 500) {
            this.logServerError(request, statusCode, exception);
        }
        ...
        if (statusCode < 500) {
            if (exceptionBody?.code) {
                errorResponse.code = exceptionBody.code;
            }

            if (exceptionBody?.resetAt) {
                errorResponse.resetAt = exceptionBody.resetAt;
            }
        }
