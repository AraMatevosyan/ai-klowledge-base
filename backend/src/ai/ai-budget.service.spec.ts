...
describe('AiBudgetService', () => {
    ...
    it('reserves budget for embeddings', async () => {
        ...
        const result =
            await service.reserveForEmbedding(
                'user-1',
                [
                    'hello',
                    'world',
    ...
    it('rejects a reservation when the accumulated daily limit is exceeded', async () => {
        ...
        const budgetError =
            caughtError as DailyAiBudgetExceededException;

        expect(
            budgetError.getStatus(),
        ).toBe(
            HttpStatus.TOO_MANY_REQUESTS,
        );

        expect(
            budgetError.getResponse(),
        ).toMatchObject({
            statusCode:
            HttpStatus.TOO_MANY_REQUESTS,
