...
export type DailyAiBudgetExceededResponse = {
    statusCode: number;
    code: typeof DAILY_AI_BUDGET_EXCEEDED_CODE;
    message: string;
    resetAt: string;
};
...
export class DailyAiBudgetExceededException extends HttpException {
    constructor(resetAt: Date) {
        const response: DailyAiBudgetExceededResponse = {
            statusCode: HttpStatus.TOO_MANY_REQUESTS,

            code: DAILY_AI_BUDGET_EXCEEDED_CODE,
    ...
    getBody(): DailyAiBudgetExceededResponse {
        return this.getResponse() as DailyAiBudgetExceededResponse;
    }
}
